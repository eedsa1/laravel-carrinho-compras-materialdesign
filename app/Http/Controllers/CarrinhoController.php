...
//necessário para poder usar os metódos de Auth
...
class CarrinhoController extends Controller
{
    ...
    function __construct()
    {
    	...
    	//a validação ocorre antes e redireciona após
    	//o login o usuário para a página que ele queria acessar
    ...
    public function index(){
        ...
        // $req = Request();
        // $data = $req->session()->get('get', 'default');
        // dd($data);
    ...
    public function adicionar(){
        ...
        //essa chamada ao middleware verifica se existe o token de segurança na requisição 
        //e se ele foi gerado pelo laravel conforme chave de sugurança do arquivo .env
    ...
    public function remover(){
        ...
        //verificação de segurança para saber se o pedido é realmente do usuário
        //e se esta no status correto para remoção. é importante isso para saber
        //se realmente o usuário tem permissão pra executar um tipo de ação
        $idpedido = Pedido::consultaId([
            'id'      => $idpedido,
            'user_id' => $idusuario,
            'status'  => 'RE'
    ...
    public function cancelar(){
        ...
        $req = Request();
        $idpedido = $req->input('pedido_id');
        //array com ids dos produtos selecionados para cancelamento
        ...
        //verificação se o pedido existe e se está como pago
        $check_pedido = Pedido::where([
            'id'      => $idpedido,
            'user_id' => $idusuario,
            'status'  => 'PA'
        ...
        //o método whereIn é utilizado com o array de ids. Ele verifica os ids.
        $check_produtos = PedidoProduto::where([
            'pedido_id' => $idpedido,
            'status'    => 'PA'
        ...
        //update de cancelamento dos pedidos/produtos
        PedidoProduto::where([
            'pedido_id' =>$idpedido,
            'status'    => 'PA'
        ...
        //veficiação para saber se restou algum item em pedidoproduto com status "pago"
        ...
        if(!$check_pedido_cancel){
            ...
            //caso toda a compra tenha sido cancelada
            $req->session()->flash('mensagem-sucesso', 'Compra cancelada com sucesso!');
        }
        else{
            //caso apenas alguns itens tenham sido cancelados
    ...
    public function desconto(){
        ...
        foreach ($pedido_produtos as $pedido_produto) {
            ...
            switch ($cupom->modo_desconto) {
                ...
                //modo por valor
                default:
                    //o valor será o especificado
                    $valor_desconto = $cupom->desconto;
                    break;
            }

            //caso o valor do produto seja menor que o do desconto (quanto for fixo) o desconto será o valor do próprio produto
            ...
            switch ($cupom->modo_limite) {
                ...
                case 'qtd':
                    $qtd_pedido =  PedidoProduto::whereIn('status', ['PA', 'RE'])->where(['cupom_desconto_id' => $cupom->id])->count();
                ...
                //quando o limite for por valor
                default:
                    $valor_ckc_descontos = PedidoProduto::whereIn('status', ['PA', 'RE'])->where(['cupom_desconto_id' => $cupom->id])->sum('desconto');
